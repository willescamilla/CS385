...
Node* create_bst_helper(int data[], int low, int high, int level) {
	...
	Node *bum = new Node(level, data[mid]);

	bum->left = create_bst_helper(data, low, mid - 1, level+1);
	if(bum->left != nullptr){
		bum->left->parent = bum;
	}

	bum->right = create_bst_helper(data, mid + 1, high, level+1);
	if(bum->right != nullptr){
		bum->right->parent = bum;
	}
